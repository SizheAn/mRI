...
class PtTransformer(nn.Module):
    ...
    def __init__(
        ...
        assert (isinstance(input_dim, list)) or (isinstance(input_dim, int))
    ...
    def preprocessing(self, video_list, padding_val=0.0):
        ...
        # less efficient yet deals with all zeros features
        if isinstance(self.input_dim, int):
            batched_masks = (batched_inputs.abs().sum(1) > 0)
        else:
            batched_inputs_splits = torch.split(batched_inputs, self.input_dim, dim=1)
            batched_masks = (batched_inputs_splits[0].abs().sum(1) > 0)
            for split in batched_inputs_splits:
                batched_masks = torch.logical_and(
                    batched_masks,
                    (split.abs().sum(1) > 0)
                )
